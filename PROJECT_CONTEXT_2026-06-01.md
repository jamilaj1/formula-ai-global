...
## 9A. Complete site map (every page)

**Totals:** 36 root pages + 41 `industries/` pages (40 sectors + index) =
**77 HTML pages**, all served from Hostinger `public_html/`. `sitemap.xml`
(auto-generated by `scripts/build_sitemap.py`) lists the public set for
search engines. Auth pages + admin carry `noindex`.

### Navigation hierarchy (what the navbar exposes)

```
jamilformula.com/  (index.html)
├─ Home ───────────────── index.html
├─ Tools ▾ (dropdown, 17 links) ──────────────────────────────────
│   ├─ AI Chat ────────── chat.html          (+ MD/PDF export)
│   ├─ Smart Search ───── search.html         (vector + keyword)
│   ├─ Workspace ──────── workspace.html      (save/edit/compare/PDF/import)
│   ├─ Calculators ────── calculators.html    (cost/scale/HLB/pH/SDS-stub)
│   ├─ Find Substitute ── substitute.html
│   ├─ Vision Scan ────── scan.html           (label/molecule/MSDS)
│   ├─ Safety ─────────── safety.html
│   ├─ Lab ────────────── lab.html
│   ├─ Predict Props ──── predict.html
│   ├─ Similarity ─────── similarity.html
│   ├─ AI Formulator ──── agent.html          (6-agent panel)
│   ├─ Compliance ─────── compliance.html
│   ├─ Encyclopedia ───── encyclopedia.html
│   ├─ Industries ─────── industries.html ──► industries/<40 sectors>.html
│   ├─ Programs ───────── programs.html
│   ├─ Contribute ─────── contribute.html
│   └─ Teach AI ───────── learn.html
├─ Consulting ────────── consulting.html      (revenue: $1k/$2.5k/$5k)
├─ Enterprise ────────── enterprise.html      (revenue: $500+/mo B2B)
├─ Pricing ───────────── pricing.html         (Pro subs $25/$50/$125)
└─ About ─────────────── about.html           (founder authority)

nav-cta / account (right side, not in the link row):
   Sign in ── login.html      Get Started ── register.html
   Dashboard ── dashboard.html
```

### Root pages by category

**Marketing / top-level (6):**
| Page | Purpose | Auth |
|---|---|---|
| index.html | Landing / hero | public |
| about.html | Founder authority (DosLunas, multi-country, career highlights) | public |
| pricing.html | Pro subscription tiers + plan compare | public |
| contact.html | Contact form + global-operations card | public |
| programs.html | University / partner programs | public |
| docs.html | Docs / API (thin — future) | public |

**AI tools (19, all in Tools dropdown):** chat, search, workspace,
calculators, substitute, scan, safety, lab, predict, similarity, agent,
compliance, encyclopedia, industries (+ 41 subpages), formulas, formula
(single-formula detail), learn, discover, contribute. Most call the
Worker (`/search` `/chat` `/library/*` `/chem/*` `/agents/*` `/vision/*`);
workspace + chat require sign-in for save/history.

**Business / revenue (2):**
| Page | Purpose | Backend |
|---|---|---|
| consulting.html | 3 packages → intake → Paystack → AI draft → delivered PDF | `/be/consulting/*` |
| enterprise.html | B2B sales page + lead form | `/be/enterprise/lead` |

**Account / auth (5):**
| Page | Purpose | Auth |
|---|---|---|
| login.html | Sign in (Supabase) | public |
| register.html | Sign up (+ auto-accept team invites by email) | public |
| dashboard.html | Signed-in home | user |
| workspace.html | Personal formula library | user |
| team.html | Multi-seat team management (Phase 9.2) | user |
| accept-invite.html | `?token=` invite landing → join team | user (noindex) |

**Internal / admin (1):** admin.html — Signups + Consulting + Financials
tabs; owner-only (auth.email gate); `noindex`. Uses a different navbar
shell (excluded from `sync_navbar.py`).

**Legal (2):** privacy.html, terms.html.

### industries/ — 40 sector landing pages (+ index)

`industries/index.html` is the hub; each sector page is SEO/landing for
that vertical and links back into search/chat scoped to the sector:

```
adhesives · agriculture · anti-aging · automotive · body-care ·
body-treatment · boiler-cooling · brightening · cleaning · coatings ·
color-cosmetics · dishwashing · disinfectants · face-makeup · face-masks ·
food-beverage · glass-ceramics · hair-care · hair-removal · heavy-industry ·
home-fragrance · household · industrial · laundry · lip-care · massage ·
metal-treatment · oral-care · paints · personal-care · personal-hygiene ·
pest-control · pet-care · pool-water · sanitation · skin-care · specialty ·
stationery · topical-analgesic · water-treatment
```

> Note: `library.html` also exists at root (saved-formulas view, predates
> the richer `workspace.html`); both read `user_formulas`. `formula.html`
> is the single-formula detail page; `formulas.html` is the browse/list.
