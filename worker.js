...
// worker-src/handlers/chemicals.js
var SELECT = "id,name,name_en,cas_number,molecular_formula,molecular_weight,category,function_category,physical_properties,synonyms,common_applications,source";
async function handleChemicals(url, env) {
...
async function handleChemicalCategories(env) {
  const r = await sbService(
    env,
  ...
  const categories = Object.entries(counts).sort((a, b) => b[1] - a[1]).slice(0, 16).map(([name, count]) => ({ name, count }));
