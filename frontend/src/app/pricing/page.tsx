    ...
    <div className="min-h-screen bg-gray-900 py-20">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {plans.map((plan) => (
            <div key={plan.name} className={'rounded-2xl p-6 ' + (plan.popular ? 'bg-green-800 ring-4 ring-green-400 scale-105' : 'bg-white/10')}>
              {plan.popular && <span className="absolute -top-3 right-4 bg-yellow-400 text-gray-900 px-3 py-1 rounded-full text-sm font-bold">Popular</span>}
              ...
              <button className={'w-full py-3 rounded-xl font-bold ' + (plan.popular ? 'bg-white text-green-700' : 'bg-white/20 text-white')}>
