        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {plans.map((plan) => (
            <div key={plan.name} className={ounded-2xl p-6 }>
              {plan.highlight && <div className="absolute -top-3 right-4 bg-yellow-400 text-gray-900 px-3 py-1 rounded-full text-sm font-bold">Popular</div>}
              ...
              <button className={w-full py-3 rounded-xl font-bold }>{plan.price === 0 ? 'Start Free' : 'Subscribe'}</button>
            </div>
          ))}
        </div>
      </div>
