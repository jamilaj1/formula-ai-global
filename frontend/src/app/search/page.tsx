...
import { Search, Copy, Check, FileSpreadsheet, FileText } from 'lucide-react'
...
export default function SearchPage() {
  ...
  const exportText = () => downloadFile('\uFEFF' + result, 'formula.txt', 'text/plain;charset=utf-8')
  ...
  return (
    <div className={`min-h-screen p-4 md:p-8 ${bg}`}>
      <div className="max-w-5xl mx-auto">
        <h1 className={`text-3xl font-bold mb-8 ${textColor}`}>{t('search')}</h1>
        ...
        <div className="flex gap-3 mb-8">
          ...
          <button onClick={handleSearch} disabled={loading}
            className="bg-green-500 text-white px-6 py-4 rounded-xl font-bold hover:bg-green-600 disabled:opacity-50">
        ...
        {result && (
          <div className="space-y-4">
            <div className={`rounded-2xl p-6 md:p-8 ${cardBg} border ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
              <pre className={`whitespace-pre-wrap font-sans leading-7 text-sm ${subColor}`}>{result}</pre>
            </div>
